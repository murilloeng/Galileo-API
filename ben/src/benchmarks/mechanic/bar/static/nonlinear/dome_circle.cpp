...
void tests::bar::static_nonlinear::dome_circle(void)
{
	...
	for(unsigned i = 0; i < 24; i++)
	{
		...
		model.mesh()->add_element(fea::mesh::elements::type::bar3, {i + n + 1, i + 2 * n + 1});
		model.mesh()->add_element(fea::mesh::elements::type::bar3, {i + 1, i != n - 1 ? i + 2 : 1});
		model.mesh()->add_element(fea::mesh::elements::type::bar3, {i + 1, i != 0 ? i + n : 2 * n});
		model.mesh()->add_element(fea::mesh::elements::type::bar3, {i + n + 1, i != n - 1 ? i + n + 2 : n + 1});
		model.mesh()->add_element(fea::mesh::elements::type::bar3, {i + n + 1, i != n - 1 ? i + 2 * n + 2 : 2 * n + 1});
	}

	//dependencies
	const double dt = M_PI / n;
//	model.boundary()->add_dependency(1, 1, fea::mesh::nodes::dof::translation_2, fea::mesh::nodes::dof::translation_1, 0, 0);
//	model.boundary()->add_dependency(n + 1, n + 1, fea::mesh::nodes::dof::translation_2, fea::mesh::nodes::dof::translation_1, 0, sin(dt) / cos(dt));
	for(unsigned i = 2; i < n + 1; i++)
	{
		const double t1 = 2 * (i - 1.0) * dt;
		const double t2 = 2 * (i - 0.5) * dt;
//		model.boundary()->add_dependency(i, 1, fea::mesh::nodes::dof::translation_3, fea::mesh::nodes::dof::translation_3);
//		model.boundary()->add_dependency(i, 1, fea::mesh::nodes::dof::translation_1, fea::mesh::nodes::dof::translation_1, 0, cos(t1));
//		model.boundary()->add_dependency(i, 1, fea::mesh::nodes::dof::translation_2, fea::mesh::nodes::dof::translation_1, 0, sin(t1));
//		model.boundary()->add_dependency(i + 24, n + 1, fea::mesh::nodes::dof::translation_3, fea::mesh::nodes::dof::translation_3);
//		model.boundary()->add_dependency(i + 24, n + 1, fea::mesh::nodes::dof::translation_1, fea::mesh::nodes::dof::translation_1, 0, cos(t2) / cos(dt));
//		model.boundary()->add_dependency(i + 24, n + 1, fea::mesh::nodes::dof::translation_2, fea::mesh::nodes::dof::translation_1, 0, sin(t2) / cos(dt));
	}
	...
	for(unsigned i = 0; i < n; i++)
	{
		model.boundary()->add_support(i + 2 * n + 1, fea::mesh::nodes::dof::translation_1);
		model.boundary()->add_support(i + 2 * n + 1, fea::mesh::nodes::dof::translation_2);
		model.boundary()->add_support(i + 2 * n + 1, fea::mesh::nodes::dof::translation_3);
	}

	//loads
	model.boundary()->add_load_case(0, fea::mesh::nodes::dof::translation_3, -1e-1 * E * A);
