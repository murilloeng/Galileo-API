...
void tests::bar::static_nonlinear::tent_unit(void)
{
	...
	const fea::mesh::nodes::dof ux = fea::mesh::nodes::dof::translation_1;
	const fea::mesh::nodes::dof uy = fea::mesh::nodes::dof::translation_2;
	const fea::mesh::nodes::dof uz = fea::mesh::nodes::dof::translation_3;
	...
	if(base)
	{
		for(unsigned i = 1; i < n; i++)
		{
			...
			model.boundary()->dependency(2 * (i - 1) + 0)->state([t](const double* d) { return cos(t) * d[0]; });
			model.boundary()->dependency(2 * (i - 1) + 1)->state([t](const double* d) { return sin(t) * d[0]; });
			model.boundary()->dependency(2 * (i - 1) + 0)->gradient([t](const double*, unsigned) { return cos(t); });
			model.boundary()->dependency(2 * (i - 1) + 1)->gradient([t](const double*, unsigned) { return sin(t); });
		}
	}
