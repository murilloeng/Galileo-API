...
static void topology_1(fea::models::Model& model)
{
	...
	model.topology()->add_point(0, tbf + hw, 0);
	...
	model.topology()->add_curve(fea::topology::curves::type::line, {2, 5});
	model.topology()->add_curve(fea::topology::curves::type::line, {5, 4});
	model.topology()->add_curve(fea::topology::curves::type::line, {4, 0});
	//surfaces
...
static void topology_2(fea::models::Model& model)
{
	...
	model.topology()->add_curve(fea::topology::curves::type::line, {6, 5});
	...
	//surfaces
	//model.topology()->add_surface({{0, 1, 2, 6, 3, 4, 5}});
}

void tests::warping::static_linear::profile_L(void)
{

	//model
	fea::models::Model model("profile L", "benchmarks/warping/static/linear");
