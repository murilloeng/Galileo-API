...
void tests::joint::pinned::static_nonlinear::multi_scissor(void)
{
	//model
	fea::models::Model model("multi scissor", "benchmarks/joints/pinned/static nonlinear");
	...
	for(unsigned i = 0; i < n; i++)
	{
		...
		((fea::mesh::joints::Pinned*) model.mesh()->joint(3 * i + 0))->stiffness(0, k);
	}
	...
	//loads
	model.boundary()->add_load_case(6 * (n - 1) + 3, fea::mesh::nodes::dof::translation_1, k / l);
