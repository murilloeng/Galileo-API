...
#include "Boundary/Loads/Elements/Mechanic/Line/Line_Force.h"
...
void tests::beam::static_linear::cantilever_uniform_force(void)
{
	//data
	const unsigned n = 10;
	...
	const double q = 1.00e+04;

	//model
	fea::models::Model model("cantilever uniform force", "benchmarks/beam/static/linear");
	...
	model.boundary()->load_case(0)->add_load_element(fea::boundary::loads::type::line_force, model.topology()->curve(0)->elements(), -q);
