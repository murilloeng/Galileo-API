...
static void create_solver(fea::models::Model* model)
{
	...
	dynamic_cast<fea::analysis::solvers::Buckling*>(model->analysis()->solver())->watch_dof(1, fea::mesh::nodes::dof::translation_3);
	model->analysis()->solve();
}

//model
void tests::deployable::buckling::slut_unit(void)
{
	//model
	fea::models::Model model("slut unit", "benchmarks/deployable/buckling");
