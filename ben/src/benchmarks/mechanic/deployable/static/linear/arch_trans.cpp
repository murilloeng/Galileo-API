...
void tests::deployable::static_linear::arch_trans(void)
{
	...
	model.boundary()->add_support(6 * 0 + 3, fea::mesh::nodes::dof::translation_1);
	model.boundary()->add_support(6 * 0 + 3, fea::mesh::nodes::dof::translation_2);
	...
	{
		model.boundary()->load_case(0)->add_load_node(6 * i + 4, fea::mesh::nodes::dof::translation_2, -Pr);
	}
