...
static void create_loads(fea::models::Model* model)
{
	...
	{
		model->boundary()->load_case(0)->add_load_node(42 * nu + 10 * ne + 2 * i + 0, uy, mat::sign(open) * pr);
		model->boundary()->load_case(0)->add_load_node(42 * nu + 10 * ne + 2 * i + 1, uy, mat::sign(open) * pr);
	}
	for(unsigned i = 0; i <= ny; i++)
	{
		model->boundary()->load_case(0)->add_load_node(42 * nu + 10 * ne + 2 * nx + 2 * (nx + 1) * i + 0, ux, -mat::sign(open) * pr);
		model->boundary()->load_case(0)->add_load_node(42 * nu + 10 * ne + 2 * nx + 2 * (nx + 1) * i + 1, ux, -mat::sign(open) * pr);
	}
}
static void create_supports(fea::models::Model* model)
{
	for(unsigned i = 0; i <= nx; i++)
	{
		model->boundary()->add_support(42 * nu + 10 * ne + 2 * (nx + 1) * ny + 2 * i + 0, uy);
		if(sups)
		{
			model->boundary()->add_support(42 * nu + 10 * ne + 2 * (nx + 1) * ny + 2 * i + 1, uy);
		}
	}
	for(unsigned i = 0; i <= ny; i++)
	{
		model->boundary()->add_support(42 * nu + 10 * ne + 2 * (nx + 1) * i + 0, ux);
		if(sups)
		{
			model->boundary()->add_support(42 * nu + 10 * ne + 2 * (nx + 1) * i + 1, ux);
		}
	}
	for(unsigned i = 0; i <= ny; i++)
	{
...
static void create_solver(fea::models::Model* model)
{
	...
	if(grav)
	{
		model->analysis()->solver()->watch_dof(42 * nu + 10 * ne, uy);
	...
	{
		model->analysis()->solver()->watch_dof(42 * nu + 10 * ne, uy);
		const double ug = model->mesh()->node(42 * nu + 10 * ne)->state(uy);
...
//model
void tests::deployable::static_nonlinear::slut_wall(void)
{
	//model
...
//	sprintf(name, "%02d", unsigned(100 * h));
	sprintf(path, "benchmarks/deployable/static/nonlinear/slut wall/models/%s", alte ? "alternate" : "uniform");
