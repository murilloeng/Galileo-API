...
void tests::deployable::static_nonlinear::tent_size(void)
{
	...
	for(unsigned i = 0; i < n; i++)
	{
		model.mesh()->add_element(fea::mesh::elements::type::beam3C, {3 * i + 1, 0});
		model.mesh()->add_element(fea::mesh::elements::type::beam3C, {3 * i + 2, 3 * i + 3});
	}
	...
	//loads
	model.boundary()->add_load_case(0, fea::mesh::nodes::dof::translation_3, -P);
