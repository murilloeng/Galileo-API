...
void tests::deployable::static_nonlinear::arch_polar(void)
{
	...
	if(base)
	{
		model.mesh()->add_element(fea::mesh::elements::type::bar2, {6 * 0 + 0, 6 * (nu - 1) + 1}, 0, 0);
	}
	...
	model.boundary()->add_support(6 * (nu - 1) + 3, fea::mesh::nodes::dof::translation_3);
	if(!base)
	{
	...
	if(unif)
	{
		for(unsigned i = 0; i < nu; i++)
		{
			...
			model.boundary()->load_case(0)->add_load_node(6 * i + 4, fea::mesh::nodes::dof::translation_2, -wr * s / 2);
			model.boundary()->load_case(0)->add_load_node(6 * i + 5, fea::mesh::nodes::dof::translation_2, -wr * s / 2);
		}
	}
	else
	{
