...
void tests::deployable::static_nonlinear::tent_unit(void)
{
	...
	for(unsigned i = 0; i < n; i++)
	{
		model.mesh()->add_element(fea::mesh::elements::type::beam3C, {2 * i + 1, 2 * i + 2});
	}
	...
	model.boundary()->add_load_case(0, fea::mesh::nodes::dof::translation_1, cos(m) * P);
	model.boundary()->add_load_case(0, fea::mesh::nodes::dof::translation_2, sin(m) * P);
