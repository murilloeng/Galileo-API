...
#include "Boundary/Loads/Load_Set.h"
...
//controls
const static int hinge = 2;
const static bool open = true;
const static bool gravity = false;
const static bool plastic = false;
const static bool friction = false;
...
const static double ka = 1.00e+03;
...
const static unsigned nm = 100;
const static unsigned ns = 800;

const static fea::mesh::elements::type element_type = fea::mesh::elements::type::beam3C;
...
static void create_joints(fea::models::Model* model)
{
	for(unsigned i = 0; i < nd; i++)
	{
		...
		for(unsigned j = 0; j < 10; j++)
		{
			...
			((fea::mesh::joints::Hinge*) model->mesh()->joint(10 * i + j))->moment_yield(friction ? mp : 1);
			((fea::mesh::joints::Hinge*) model->mesh()->joint(10 * i + j))->plastic_modulus(friction ? kp : 0);
			((fea::mesh::joints::Hinge*) model->mesh()->joint(10 * i + j))->stiffness(0, 2, friction ? ka : 0);
		}
	}
}
...
static void create_elements(fea::models::Model* model)
{
	for(unsigned i = 0; i < nd; i++)
	{
		...
		const unsigned mat_inner = plastic;
		const double t = (2 * i + 1) * M_PI / nd;
		const double s2[] = {-sin(t), +cos(t), 0};
		const double u1[] = {+cos(t - M_PI / nd), +sin(t - M_PI / nd), 0};
		const double e1[] = {+cos(t + M_PI / nd), +sin(t + M_PI / nd), 0};
		const double* bo[] = {s2, s2, s2, s2, s2, s2, e1, e1, s2, u1, e1, s2, u1, e1, e1, e1};
		model->mesh()->add_element(element_type, {22 * i +  2, 0}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i +  4, 1}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i +  3, 22 * i +  6}, mat_inner);
		model->mesh()->add_element(element_type, {22 * i +  5, 22 * i +  7}, mat_inner);
		model->mesh()->add_element(element_type, {22 * i +  6, 22 * i + 14}, mat_inner);
		model->mesh()->add_element(element_type, {22 * i +  7, 22 * i + 21}, mat_inner);
		model->mesh()->add_element(element_type, {22 * i +  8, 22 * i + 16}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i +  9, 22 * i + 23}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 10, 22 * i + 11}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 10, 22 * i + 12}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 10, 22 * i + 13}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 17, 22 * i + 18}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 17, 22 * i + 19}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i + 17, 22 * i + 20}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i +  8, 22 * (i + 1 != nd ? i + 1 : 0) + 22}, mat_outer);
		model->mesh()->add_element(element_type, {22 * i +  9, 22 * (i + 1 != nd ? i + 1 : 0) + 15}, mat_outer);
...
static void create_loads(fea::models::Model* model)
{
	...
	//load set
	if(gravity) model->boundary()->load_set(0)->add_set_case(true, 1, 1);
}
...
//model
void tests::deployable::static_nonlinear::slut_unit(void)
{
	//model
	fea::models::Model model("slut unit", "benchmarks/deployable/static/nonlinear");
