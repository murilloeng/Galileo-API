...
//load
const static std::function<double(double)> f = [] (double t) { return sin(t); };
...
static void create_solver(fea::models::Model* model)
{
	...
	model->analysis()->solver()->step_max(ns);
	model->analysis()->solve();
}

//model
void tests::deployable::dynamic_linear::slut_unit(void)
{
	//model
	fea::models::Model model("slut unit", "benchmarks/deployable/dynamic/linear");
