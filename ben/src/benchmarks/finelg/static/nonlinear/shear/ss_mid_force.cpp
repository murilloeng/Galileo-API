...
void tests::finelg::static_nonlinear::shear::ss_mid_force(void)
{
	/*
	simple supported beam subjected to mid force
	*/
	...
	//model
	fea::models::Model model("SS mid force", "benchmarks/finelg/static/nonlinear/shear");
	...
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->load_max(2e4);
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->step_max(100);
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->load_guess(2e2);
