...
void tests::finelg::static_nonlinear::shear::cf_tip_force(void)
{
	/*
	campled-free beam subjected to tip force
	*/
	...
	//model
	fea::models::Model model("CF tip force", "benchmarks/finelg/static/nonlinear/shear");
	...
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->load_max(2e3);
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->step_max(100);
	dynamic_cast<fea::analysis::solvers::Static_Nonlinear*>(model.analysis()->solver())->load_guess(2e1);
